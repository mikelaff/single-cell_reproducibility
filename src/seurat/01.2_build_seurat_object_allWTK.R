...
# OUTPUT FILES #########################################################################################################
# merged sce
sce.merged.rds <- here("results/alevin_fry/20230217_WTK1-3-4-5-6_merged_sce.rds")

# merged seurat file
seurat.merged.rds <- here("results/seurat/20230217_WTK1-3-4-5-6_seurat_object.rds")
...
# remove WTK2, memory issues preventing building full dataset
df.sublibraries %<>%
    filter(!WTK_ID == "WTK2")
...
for (i in 1:nrow(df.sublibraries)) {
    ...
    df.coldata %<>%
        mutate(Sublibrary_ID = df.sublibraries$Sublibrary_ID[i])
...
df.cellData %<>%
    left_join(dplyr::select(df.samples, CODissoID, WTK_ID), by = c("WTK_ID", "CODissoID"))
...
df.cellData %<>%
    mutate(cell_name = paste(barcodes, WTK_ID, Sublibrary, CODissoID, sep = "_"))
...
seurat.merged <- CreateSeuratObject(counts = counts(sce.merged), project = "ALL_Samples", meta.data = df.cellData)
